...
def display_food_database(food_database):
    for food in food_database:
        ...
        print(f"Calories per Serving: {food['calories']} cal/serving")
        print(f"Carbohydrates per Serving: {food['carbohydrates']}g/serving")
        print(f"Proteins per Serving: {food['proteins']}g/serving")
        print(f"Fats per Serving: {food['fats']}g/serving")
        print()
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            servings = float(input("Enter the number of servings: "))
            meal_category = input("Enter the meal category (breakfast/lunch/dinner/snacks): ")
            ...
            if food is not None:
                ...
                meal_log[meal_category.lower()].append(meal_entry)
                print(f"{food_name} added to {meal_category} in the meal log.")
            else:
                print("Food not found in the database.")
        elif choice == '4':
            if any(meal_log.values()):
                print("Meal Log:")
                for category, entries in meal_log.items():
                    if entries:
                        print(f"{category.capitalize()}:")
                        for entry in entries:
                            print(f"Name: {entry['name']}")
                            print(f"Calories: {entry['calories']} cal")
